 ...
 * @access 		public
 */
class TwitterBombFormSelectCampaigns extends XoopsFormElement
{
    /**
     * Options
     *
     ...
     */
    function TwitterBombFormSelectCampaigns($caption, $name, $value = null, $size = 1, $multiple = false, $ownid=0, $type = 'bomb,scheduler,retweet,reply,mentions,pictures')
    {
		...
		@$this->GetCampaign($ownid, $type);
    }

	function GetCampaign($ownid, $type){
		...
		$criteriac = new CriteriaCompo($criteriaa, 'OR');
		$criteriac->add($criteriab, 'OR');
		$criteria = new CriteriaCompo($criteriac, 'AND');
		$criteria->add(new Criteria('`type`', '("'.implode('","', explode(',', $type)).'")', 'IN'));
		$campaigns = $campaign_handler->getObjects($criteria, true);
		foreach($campaigns as $cid => $campaign) {
			if ($cid!=$ownid&&$ownid>0)
				$this->addOption($cid, $campaign->getVar('name'));
			else
				$this->addOption($cid, $campaign->getVar('name'));
		}
