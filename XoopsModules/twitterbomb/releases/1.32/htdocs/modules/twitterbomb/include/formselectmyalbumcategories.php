 ...
 * @access 		public
 */
class TwitterBombFormSelectMyalbumCategories extends XoopsFormElement
{
    /**
     * Options
     *
     ...
     */
    function TwitterBombFormSelectMyalbumCategories($caption, $name, $value = null, $size = 1, $multiple = false, $ownid=0)
    {
		...
		foreach($this->GetCategories($ownid) as $id => $lang) 
			$this->addOption($id, ($lang));
    }

	function GetCategories($ownid){
	
		$category_handler =& xoops_getmodulehandler('cat', 'myalbum');
		$categories = $category_handler->getObjects(new Criteria('pid', 0), true);
	...
	function TreeMenu($langs_array, $categories, $level, $ownid) {
		...
		foreach($categories as $catid => $category) {
			if ($catid!=$ownid) {
				$langs_array[$catid] = str_repeat('--', $level).$category->getVar('title');
