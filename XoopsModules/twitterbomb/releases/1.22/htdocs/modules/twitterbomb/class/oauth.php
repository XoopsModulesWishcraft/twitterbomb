...
class TwitterbombOauth extends XoopsObject
{
	...
	private function setLimits($limits) {
		$sql = "UPDATE ".$GLOBALS['xoopsDB']->prefix('twitterbomb_oauth').' SET `remaining_hits` = "'.$limits['remaining_hits'].'", `hourly_limit` = "'.$limits['hourly_limit'].'", `api_resets` = "'.$limits['reset_time_in_seconds'].'", `updated` = "'.time().'" WHERE `oid` = ' . $this->getVar('oid');
		$GLOBALS['xoopsDB']->queryF($sql);
