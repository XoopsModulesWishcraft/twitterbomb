<?php
	...
	switch($op) {
		...
		case "scheduler":	
			switch ($fct)
			{
				...
				case "list":				
					...
					$ttl = $scheduler_handler->getCount(NULL);
					$limit = !empty($_REQUEST['limit'])?intval($_REQUEST['limit']):30;
					$start = !empty($_REQUEST['start'])?intval($_REQUEST['start']):0;
					$order = !empty($_REQUEST['order'])?$_REQUEST['order']:'DESC';
				...
				case "save":
					...
					if (!$id=$scheduler_handler->insert($scheduler)) {
						redirect_header('index.php?op='.$op.'&fct=list', 10, _AM_MSG_SCHEDULER_FAILEDTOSAVE);
						exit(0);
					} else {
				...
				case "savelist":
					...
					redirect_header('index.php?op='.$op.'&fct=list', 10, _AM_MSG_SCHEDULER_SAVEDOKEY);
					exit(0);
					break;				
				case "delete":	
					...
					if (isset($_POST['id'])&&$id=intval($_POST['id'])) {
						...
						if (!$scheduler_handler->delete($scheduler)) {
							redirect_header('index.php?op='.$op.'&fct=list', 10, _AM_MSG_SCHEDULER_FAILEDTODELETE);
							exit(0);
						} else {
							redirect_header('index.php?op='.$op.'&fct=list', 10, _AM_MSG_SCHEDULER_DELETED);
							exit(0);
						}
					} else {
						$scheduler = $scheduler_handler->get(intval($_REQUEST['id']));
						xoops_confirm(array('id'=>$_REQUEST['id'], 'op'=>$_REQUEST['op'], 'fct'=>$_REQUEST['fct']), 'index.php', sprintf(_AM_MSG_SCHEDULER_DELETE, $scheduler->getVar('text')));
					}
					break;
				case "importfile":
					    	...
					    	set_time_limit(3600);
					      	...
					      	foreach($lines as $line) {
								...
								if (!$id=$scheduler_handler->insert($scheduler)) {
									...
									print_r($scheduler);
									exit(0);
					      			redirect_header('index.php?op='.$op.'&fct=list', 10, _AM_MSG_SCHEDULER_IMPORT_FAILED);
					      			exit(0);
					      		}
					      	}
					      	...
					      	redirect_header('index.php?op='.$op.'&fct=list', 10, _AM_MSG_SCHEDULER_IMPORT_SUCCESSFUL);
					      	exit(0);
					    }
					  } else {
					    echo $uploader->getErrors();
		...
		case "log":	
			...
			foreach (array(	'provider','date','alias','tweet','url') as $id => $key) {
				$GLOBALS['xorTpl']->assign(strtolower(str_replace('-','_',$key).'_th'), '<a href="'.$_SERVER['PHP_SELF'].'?start='.$start.'&limit='.$limit.'&sort='.str_replace('_','-',$key).'&order='.((str_replace('_','-',$key)==$sort)?($order=='DESC'?'ASC':'DESC'):$order).'&op='.$op.'">'.(defined('_AM_TWEETBOMB_TH_'.strtoupper(str_replace('-','_',$key)))?constant('_AM_TWEETBOMB_TH_'.strtoupper(str_replace('-','_',$key))):'_AM_TWEETBOMB_TH_'.strtoupper(str_replace('-','_',$key))).'</a>');
			}
