		...
		while(count($ret)<$items) {
			...
			if (strlen($username)>0) {
				...
				if ($GLOBALS['xoopsModuleConfig']['htaccess']==false)
					$ret[$c]['link'] = XOOPS_URL.'/modules/twitterbomb/go.php?cid='.$cid.'&catid='.$catid.'&uri='.urlencode(sprintf($url, urlencode($sentence)));
				else 
					$ret[$c]['link'] = XOOPS_URL.'/'.$GLOBALS['xoopsModuleConfig']['baseurl'].'/go'.$GLOBALS['xoopsModuleConfig']['endofurl'].'?cid='.$cid.'&catid='.$catid.'&uri='.urlencode(sprintf($url, urlencode($sentence)));
