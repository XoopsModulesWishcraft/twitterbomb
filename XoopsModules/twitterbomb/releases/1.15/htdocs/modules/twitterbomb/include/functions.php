...
if (!function_exists('twitterbomb_get_rss')) {
	function twitterbomb_get_rss($items, $cid, $catid) {
		...
		while(count($ret)<$items&&$looped<$items*2) {
			...
			if (count($ret)<$items) {
				$mtr=mt_rand(0,5);
				$ret[$c]['title'] = (strlen($username)>0&&$mtr>2?'@'.$username.' ':'').str_replace('#@', '@', str_replace('#(', '(#', str_replace('##', '#', twitterbomb_TweetString(htmlspecialchars_decode($sentence), $GLOBALS['xoopsModuleConfig']['aggregate'], $GLOBALS['xoopsModuleConfig']['wordlength']))));	  
				...
				$ret[$c]['description'] = (strlen($username)>0&&$mtr>2?'@'.$username.' ':'').htmlspecialchars_decode($sentence);
