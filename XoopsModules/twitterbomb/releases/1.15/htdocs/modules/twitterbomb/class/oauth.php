...
class TwitterbombOauth extends XoopsObject
{
	...
	private function reset() {
		$sql = "UPDATE ".$GLOBALS['xoopsDB']->prefix('twitterbomb_oauth').' SET `calls` = 0, `reset` = "'.time().'" WHERE `oid` = ' . $this->getVar('oid');
	...
	private function setLimits($limits) {
		$sql = "UPDATE ".$GLOBALS['xoopsDB']->prefix('twitterbomb_oauth').' SET `remaining_hits` = "'.$limits['remaining_hits'].'", `hourly_limit` = "'.$limits['hourly_limit'].'", `api_resets` = "'.$limits['reset_time_in_seconds'].'" WHERE `oid` = ' . $this->getVar('oid');
		$GLOBALS['xoopsDB']->queryF($sql);
	...
	private function increaseCall($amount=1) {
		$sql = "UPDATE ".$GLOBALS['xoopsDB']->prefix('twitterbomb_oauth').' SET `calls` = `calls` + '.$amount.' WHERE `oid` = ' . $this->getVar('oid');
		$GLOBALS['xoopsDB']->queryF($sql);
		$this->vars['calls']['value']=$this->vars['calls']['value']+$amount;
	}
	
	private function increaseTweets($amount=1) {
		$sql = "UPDATE ".$GLOBALS['xoopsDB']->prefix('twitterbomb_oauth').' SET `tweets` = `tweets` + '.$amount.', `tweeted` = "'.time().'" WHERE `oid` = ' . $this->getVar('oid');
		$GLOBALS['xoopsDB']->queryF($sql);
	...
	private function validate($user) {
		$sql = "UPDATE ".$GLOBALS['xoopsDB']->prefix('twitterbomb_oauth').' SET `mode` = "valid", `username` = "'.$user['screen_name'].'", `id` = "'.$user['id'].'" WHERE `oid` = ' . $this->getVar('oid');
		$GLOBALS['xoopsDB']->queryF($sql);
	...
	function setFriendsTimer() {
		$sql = "UPDATE ".$GLOBALS['xoopsDB']->prefix('twitterbomb_oauth').' SET `friends` = "'.time()+$this->_modConfig['look_for_friends'].'" WHERE `oid` = ' . $this->getVar('oid');
		$GLOBALS['xoopsDB']->queryF($sql);
		$this->vars['friends']['value']=time()+$this->_modConfig['look_for_friends'];
	}

	function setMentionsTimer() {
		$sql = "UPDATE ".$GLOBALS['xoopsDB']->prefix('twitterbomb_oauth').' SET `mentions` = "'.time()+$this->_modConfig['look_for_mention'].'" WHERE `oid` = ' . $this->getVar('oid');
		$GLOBALS['xoopsDB']->queryF($sql);
	...
	function validateCredentials($for_tweet=false) {
		...
		if (is_a($this->_connection, 'TwitterOAuth')&&$this->getVar('remaining_hits')>0) {
			$user = twitterbomb_object2array($this->_connection->get('account/verify_credentials', 	array(	'include_entities' => 'true'	)));
			switch ($this->_connection->http_code) {
		  		case 200:
		    		$this->increaseCall(1);
		    		$this->validate($user);
	...
	function getUser($mixed = '', $type='user_id', $for_tweet=false) {
		...
		if (is_a($this->_connection, 'TwitterOAuth')&&$this->getVar('remaining_hits')>0) {
			$user = twitterbomb_object2array($this->_connection->get('users/show', 	array(	$type=>$mixed, 'include_entities' => 'true'	)));
			switch ($this->_connection->http_code) {
		  		case 200:
		    		$this->increaseCall(1);
	    			return $user;
