		...
		if ($c<=$xoConfig['tweets_per_session']) {
			switch($campaign->getVar('type')) {
				case "bomb":
					$items = 0;
					$ret = array();
					while($xoConfig['items']<$items&&$c<=$xoConfig['tweets_per_session']) {
						...
						if (strlen($username)>0) {
							$mtr=mt_rand(0,5);
							$tweet = (strlen($username)>0&&$mtr>2?'@'.$username.' ':'').str_replace('#@', '@', str_replace('#(', '(#', str_replace('##', '#', twitterbomb_TweetString(htmlspecialchars_decode($sentence), $xoConfig['aggregate'], $xoConfig['wordlength']))));
				...
				case "scheduler":
					$items=0;
					$ret = array();
					while($xoConfig['scheduler_items']>$items&&$c<=$xoConfig['tweets_per_session']) {
						$sentence = $scheduler_handler->getTweet($cid, $catid, 0, 0);
						if (is_array($sentence)) {
							$url = $urls_handler->getUrl($cid, $catid);
							$tweet = str_replace('#@', '@', str_replace('#(', '(#', str_replace('##', '#', twitterbomb_TweetString(htmlspecialchars_decode($sentence['tweet']), $xoConfig['scheduler_aggregate'], $xoConfig['scheduler_wordlength']))));	  
