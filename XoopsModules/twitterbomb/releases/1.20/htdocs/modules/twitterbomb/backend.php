			...
			if (!$sarray = XoopsCache::read('tweetbomb_'.$campaign->getVar('type').'_'.$cacheid)) {
				switch ($campaign->getVar('type')) {
					...
					case 'scheduler':
			    		...
			    		break;
					case 'retweet':
						$sarray = twitterbomb_get_retweet_rss($GLOBALS['xoopsModuleConfig']['retweet_items'], $cid, $catid);
			    		XoopsCache::write('tweetbomb_'.$campaign->getVar('type').'_'.$cacheid, $sarray, $GLOBALS['xoopsModuleConfig']['retweet_cache']);
			    		break;	
				} 
		    }
