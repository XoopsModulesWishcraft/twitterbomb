...
class TwitterBombRetweet extends XoopsObject
{
    ...
    function TwitterBombRetweet($id = null)
    {
...
class TwitterBombRetweetHandler extends XoopsPersistableObjectHandler
{
    ...
    function getFilterForm($filter, $field, $sort='created', $op = 'retweet', $fct='list') {
    ...
    function doSearchForTweet($cid, $catid, $rids) {
    	...
    	foreach($this->getObjects($criteria, true) as $rid => $retweet) {
    		...
    		// Do Search
    		$ret[$rid] = twitterbomb_searchtwitter($this->_modConfig['gather_on_search'], $retweet->getVar('search'), explode('|',$retweet->getVar('skip')), ($retweet->getVar('geocode')==true?$retweet->getVar('longitude').','.$retweet->getVar('latitude').','.$retweet->getVar('radius').$retweet->getVar('measurement'):''), $retweet->getVar('language'), 1, $retweet->getVar('type'), ($this->_modConfig['gather_on_search']<100?$this->_modConfig['gather_on_search']:'100'), 'true', '', $since_id);
