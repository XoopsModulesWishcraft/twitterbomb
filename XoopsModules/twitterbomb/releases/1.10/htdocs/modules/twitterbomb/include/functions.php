	...
	function twitterbomb_get_rss($items, $cid, $catid) {
		...
		while(count($ret)<$items) {
			...
			if (strlen($username)>0) {
				$ret[$c]['title'] = (strlen($username)>0&&mt_rand(0,1)==1?'@'.$username.' ':'').twitterbomb_TweetString($sentence, $GLOBALS['xoopsModuleConfig']['aggregate'], $GLOBALS['xoopsModuleConfig']['wordlength']);	  
